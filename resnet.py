...
def conv_block(x, n_filters1, n_filters2, n_blocks, layer, initializer=keras.initializers.HeNormal()):
  ...
  for n in range(n_blocks):
    # after layer 2, at the beginning of each layer, we want to downsample the input with 1x1 convolutions, and strides of 2
    start_strides = 2 if (layer>2 and n==0) else 1
    x = keras.layers.Conv2D(filters=n_filters1, kernel_size=(1,1), strides=start_strides, padding='same', kernel_initializer=initializer)(x)
    x = bn_relu(x)
    # in every other case, strides are 1
    x = keras.layers.Conv2D(filters=n_filters1, kernel_size=(3,3), padding='same', kernel_initializer=initializer)(x)
    x = bn_relu(x)
    x = keras.layers.Conv2D(filters=n_filters2, kernel_size=(1,1), padding='same', kernel_initializer=initializer)(x)
    x = keras.layers.BatchNormalization()(x)

    # if we are at the beginning of the block, the channel dimension of the original input should match the dim. of our current x
    if n==0:
      identity = keras.layers.Conv2D(filters=n_filters2, kernel_size=(1,1), strides=start_strides, padding='same', kernel_initializer=initializer)(identity)
...
def resnet(input_size, n_classes):
  ...
  # conv1_x
  x = keras.layers.Conv2D(filters=64, kernel_size=(7,7), strides=2, padding='same', kernel_initializer=initializer)(inputs)
  x = bn_relu(x)
